...
from app import db
from app.models import TestPlan, TestPlanCase, TestExecution, TestCase
...
test_plan_model = test_plan_ns.model('TestPlan', {
    ...
    'automation_enabled': fields.Boolean(description='是否启用自动化'),
    'agent_config': fields.String(description='Agent配置(JSON)')
})
    ...
    def post(self):
        ...
        try:
            ...
            test_plan = TestPlan(
                ...
                agent_config=json.dumps(data.get('agent_config')) if data.get('agent_config') else None,
...
class TestPlanAPI(Resource):
    ...
    def put(self, plan_id):
        ...
        try:
            ...
            test_plan.automation_enabled = data.get('automation_enabled', test_plan.automation_enabled)
            ...
            if data.get('agent_config') is not None:
                test_plan.agent_config = json.dumps(data.get('agent_config'))
...
class TestPlanExecuteAPI(Resource):
    ...
    def post(self, plan_id):
        ...
        try:
            ...
            # 检查是否启用自动化
            if test_plan.automation_enabled:
                # TODO: 调用自动化Agent执行
                # 这里预留Agent对接接口
                return success_response(
                    data={'execution_id': None, 'status': 'pending'},
                    message='自动化执行任务已提交'
                )
            else:
                # 手动执行模式
